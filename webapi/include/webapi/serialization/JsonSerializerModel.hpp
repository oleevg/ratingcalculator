...
namespace rating_calculator {
  ...
  namespace webapi {
    ...
    namespace serialization {
      ...
      template <> struct JsonSerializer<core::UserInformation> {
      ...
      template <> struct JsonSerializer<core::DealInformation> {
      ...
      template <> struct JsonSerializer<core::UserIdInformation> {
      ...
      template <> struct JsonSerializer<core::UserPosition> {
        static boost::property_tree::ptree Serialize(const core::UserPosition& value)
        {
          boost::property_tree::ptree result;

          result.add_child("userInfo",
                           JsonSerializer<decltype(value.userInformation)>::Serialize(value.userInformation));
      ...
      template <> struct JsonSerializer<core::UserRelativeRating> {
        static boost::property_tree::ptree Serialize(const core::UserRelativeRating& value)
        {
          ...
          result.add_child("headPositions",
                           JsonSerializer<decltype(value.headPositions)>::Serialize(value.headPositions));
          result.add_child("highPositions",
                           JsonSerializer<decltype(value.highPositions)>::Serialize(value.highPositions));
      ...
      template <class T> struct JsonSerializer<core::Message<T>> {
      ...
      template <> struct JsonSerializer<core::BaseMessage> {
        static boost::property_tree::ptree Serialize(const core::BaseMessage& value)
        {
          ...
          else
          {
            ...
            throw SerializerException((boost::format("Can't serialize unsupported message '%s'.") %
                                       enumConverter.toString(value.getType()).c_str())
                                          .str());
          }
