...
namespace rating_calculator {
  ...
  namespace core {
     ...
     */
    template <class AccessKey, class SortKey, class Value, AccessKey Value::* accessKey, SortKey Value::* sortKey,
    ...
    private:
      typename SortContainer::iterator getSortNode(const Value& value)
      {
        ...
        if (accessIter == accessData_.end())
        {
          throw core::BaseException(
              (boost::format("Item with access key '%s' is not present in the access container.") % value.*accessKey)
                  .str());
        }
      ...
      typename SortContainer::const_iterator findInSortContainer(const ValuePtr& valuePtr) const
      {
        ...
        while ((**iter).*accessKey != *valuePtr.*accessKey)
        {
          ...
          {
            throw core::BaseException(
                (boost::format("Item with access key '%s' is not present in the sort container.") %
                 *valuePtr.*accessKey)
                    .str());
          }
      ...
      typename SortContainer::const_iterator findInSortContainer(const AccessKey& key) const
      {
        ...
        {
          throw core::BaseException(
              (boost::format("Item with access key '%s' is not present in access container.") % key).str());
        }
